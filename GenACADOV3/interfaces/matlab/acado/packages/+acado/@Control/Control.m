...
%  Usage:
%    >> Control(name);
%    >> Control name;                          SHORTHAND
%    >> Controls name1 name2 name3 ....        SHORTHAND
%
%  Parameters:
...
% 
%
classdef Control < acado.Variable
    properties(SetAccess='private')
        
    end
    
    methods
        function obj = Control(name)
            if nargin > 0
                global ACADO_;
            ...
            for j = 1:length(obj)
                for i = 1:length(var)
                    ...
                    if isa(var(i), 'acado.Variable')
                        jac(j,i) = acado.DoubleConstant(double(isa(var(i), 'acado.Control') && strcmp(obj(j).name, var(i).name)));
                    else
